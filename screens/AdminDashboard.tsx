...
const AdminDashboard: React.FC = () => {
  ...
  return (
    <div className="h-screen bg-gray-50 text-gray-900 flex flex-col font-manrope overflow-hidden">
      <header className="px-6 py-5 border-b border-gray-200 flex items-center justify-between bg-navy-card/50 backdrop-blur-sm z-20">
        <div className="flex items-center gap-4">
          <button onClick={() => navigate('/profile')} className="size-10 flex items-center justify-center rounded-xl bg-gray-100 border border-gray-200">
            <span className="material-symbols-outlined text-gray-500">arrow_back</span>
          </button>
          <div>
            <h1 className="text-lg font-black tracking-tight italic">COMMAND CENTER</h1>
            <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest mt-0.5">실시간 운영 지표 및 피드백 로그</p>
          </div>
        </div>
      </header>

      <nav className="flex px-4 bg-navy-card/30 border-b border-gray-200 z-10">
        {[
          ...
          <button
            ...
            className={`flex items-center gap-2 px-6 py-5 text-[10px] font-black uppercase tracking-widest relative ${activeTab === tab.id ? 'text-primary bg-gray-100' : 'text-gray-400'}`}
          >
        ...
        {activeTab === 'stats' && (
          <div className="space-y-8 animate-in fade-in duration-500">
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-navy-card border border-gray-200 p-6 rounded-[2.5rem] shadow-xl">
                 <p className="text-[9px] font-black text-gray-400 uppercase tracking-widest mb-2">리포트 만족도 (MVP)</p>
                 <div className="flex items-baseline gap-2">
                    <h2 className="text-3xl font-black text-gray-900">{stats.satisfaction}%</h2>
                    <span className="text-[10px] text-primary font-bold">POSITIVE</span>
                 </div>
              </div>
              <div className="bg-navy-card border border-gray-200 p-6 rounded-[2.5rem] shadow-xl">
                 <p className="text-[9px] font-black text-gray-400 uppercase tracking-widest mb-2">총 훈련 수</p>
                 <div className="flex items-baseline gap-2">
                    <h2 className="text-3xl font-black text-gray-900">{leaders.length * 3}</h2>
          ...
          <div className="space-y-4 animate-in fade-in duration-500">
            ...
            {feedbackLogs.length > 0 ? (
              feedbackLogs.slice().reverse().map((log) => (
                <div key={log.id} className="bg-navy-card p-5 rounded-[2rem] border border-gray-200 flex items-center justify-between">
                   <div className="flex items-center gap-4">
                      ...
                      <div>
                         <p className="text-sm font-bold text-gray-900">{log.scenarioId || '알 수 없는 시나리오'}</p>
                         <p className="text-[9px] text-gray-400 uppercase">{new Date(log.timestamp).toLocaleString()}</p>
                      </div>
                   </div>
                   <span className="text-[10px] font-black text-gray-400">ID: {log.id.toString().slice(-4)}</span>
                </div>
              ))
            ) : (
