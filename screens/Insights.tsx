...
const Insights: React.FC = () => {
  ...
  return (
    <div className="min-h-[100dvh] flex flex-col bg-gray-50 text-gray-900 font-manrope lg:px-12">
      <header className="p-5 bg-white/80 backdrop-blur-sm border-b border-gray-200 flex items-center justify-between sticky top-0 z-30 lg:px-0">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[9px] text-gray-400 font-bold uppercase tracking-widest mt-1">Org-wide Benchmark</p>
          </div>
        </div>
        <button onClick={() => navigate('/admin')} className="size-10 bg-gray-100 rounded-xl flex items-center justify-center border border-gray-200 active:scale-90 transition-all hover:bg-primary/10 group">
          <span className="material-symbols-outlined text-gray-400 group-hover:text-primary transition-colors">admin_panel_settings</span>
        </button>
      </header>
      ...
      <main className="flex-1 p-6 space-y-8 pb-32 hide-scrollbar lg:px-0 lg:py-12">
        <div className="lg:grid lg:grid-cols-12 lg:gap-8 space-y-8 lg:space-y-0">
          ...
          <div className="lg:col-span-5 space-y-8">
            <div className="bg-gradient-to-br from-primary/20 to-navy-card border border-primary/30 p-8 rounded-[3rem] shadow-card relative overflow-hidden group">
               ...
               <div className="flex items-end gap-3 mb-4">
                  <h1 className="text-4xl sm:text-5xl font-black text-gray-900 italic tracking-tighter">{userScore}</h1>
                  <span className="text-[10px] sm:text-xs font-bold text-primary mb-2">상위 12% 리더</span>
               </div>
               <p className="text-xs sm:text-sm text-gray-600 font-medium leading-relaxed">
                 리더님은 우리 회사 평균(<span className="text-gray-900 font-bold">{orgStats.avgLQ}점</span>)보다 <span className="text-primary font-bold">{userScore - orgStats.avgLQ}점</span> 높게 달리고 있어요!
               </p>
            </div>

            <div className="bg-navy-card rounded-[3rem] p-6 sm:p-8 border border-gray-200 shadow-card">
              <div className="flex justify-between items-center mb-8 px-2">
                 <h3 className="text-[10px] font-bold uppercase text-gray-400 tracking-[0.2em]">나의 5대 역량 그래프</h3>
                 <button 
              ...
              <div className="flex justify-center mb-10 relative">
                <svg className="w-full max-w-[280px] sm:max-w-[320px] aspect-square overflow-visible" viewBox="0 0 100 100">
                  ...
                  <defs>
                    <radialGradient id="radarGradient">
                      <stop offset="0%" stopColor="rgba(49, 130, 246, 0.1)" />
                      <stop offset="100%" stopColor="rgba(49, 130, 246, 0.5)" />
                    </radialGradient>
                  </defs>
                  {[
                ...
                <div className="absolute -bottom-4 left-1/2 -translate-x-1/2 flex gap-4">
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="text-[9px] font-bold text-gray-600">리더님</span>
                  </div>
                  <div className="flex items-center gap-1.5">
                    <div className="size-2 rounded-full bg-gray-200 border border-white/40 border-dashed"></div>
          ...
          <div className="lg:col-span-7 space-y-8">
            <div className="bg-navy-card rounded-[3rem] p-6 sm:p-8 border border-gray-200 shadow-card relative overflow-hidden">
               <div className="flex justify-between items-start mb-8">
                  <div>
                    ...
                    <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest">나의 기록 vs 회사 평균</p>
                  </div>
               </div>
                    ...
                    return (
                      <div key={idx} className="flex flex-col items-center gap-4 group flex-1">
                         <div className="relative w-full flex items-end justify-center gap-1">
                            <div className="w-2.5 sm:w-5 bg-gray-100 rounded-t-sm" style={{ height: animate ? `${orgHeight}px` : '0px', transition: 'height 1s ease-out' }}></div>
                         ...
                         <span className="text-[9px] font-bold text-gray-400">{data.month}</span>
                      </div>
                    );
                  })}
               </div>
            ...
            <div className="bg-navy-card rounded-[3rem] p-6 sm:p-8 border border-gray-200 shadow-card">
              <div className="flex justify-between items-start mb-6">
                <div>
                  ...
                  <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest">Session Growth Trend</p>
                </div>
              </div>
              ...
              <div className="flex flex-wrap gap-2 mb-6">
                {([
                  { key: 'trustLevel' as const, label: '신뢰도', color: '#3182F6' },
                  ...
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-full text-[10px] font-bold border transition-all ${
                      selectedMetric === m.key
                        ...
                        : 'border-gray-200 bg-white/[0.02] text-gray-400 hover:bg-gray-100'
                    }`}
                  >
                ...
                <div className="relative h-48 sm:h-56">
                  ...
                  {[0, 25, 50, 75, 100].map(v => (
                    <div key={v} className="absolute w-full flex items-center" style={{ bottom: `${v}%` }}>
                      <span className="text-[8px] text-gray-400 w-7 text-right mr-2">{v}</span>
                    ...
                    <defs>
                      <linearGradient id="growthFill" x1="0" y1="0" x2="0" y2="1">
                        <stop offset="0%" stopColor={
                          selectedMetric === 'trustLevel' ? '#3182F6' :
                  ...
                  <div className="absolute bottom-[-20px] left-9 right-0 flex justify-between">
                    {growthData.map((d, i) => (
                      <span key={i} className="text-[8px] text-gray-400 font-bold">{d.label}</span>
                    ))}
                  </div>
                </div>
              ) : (
                <div className="flex flex-col items-center justify-center py-12 gap-3">
                  <span className="material-symbols-outlined text-3xl text-gray-400">show_chart</span>
                  <p className="text-xs text-gray-400 font-medium">2회 이상 코칭을 완료하면 성장 추이가 표시됩니다</p>
                </div>
              )}
              ...
              {growthData.length >= 2 && (
                <div className="mt-10 grid grid-cols-3 gap-3">
                  {(() => {
                    ...
                    return (
                      <>
                        <div className="bg-white/[0.03] rounded-2xl p-4 text-center border border-gray-200">
                          <p className="text-[9px] text-gray-400 font-bold mb-1">변화량</p>
                          <p className={`text-lg font-black ${diff >= 0 ? 'text-primary' : 'text-red-400'}`}>
                            {diff >= 0 ? '+' : ''}{diff}
                          </p>
                        </div>
                        <div className="bg-white/[0.03] rounded-2xl p-4 text-center border border-gray-200">
                          <p className="text-[9px] text-gray-400 font-bold mb-1">최고 점수</p>
                          <p className="text-lg font-black text-gray-900">{max}</p>
                        </div>
                        <div className="bg-white/[0.03] rounded-2xl p-4 text-center border border-gray-200">
                          <p className="text-[9px] text-gray-400 font-bold mb-1">평균</p>
                          <p className="text-lg font-black text-gray-900">{avg}</p>
                        </div>
                      </>
                    );
                  })()}
      ...
      {showProfileDetail && (
        <div className="fixed inset-0 z-[100] bg-black/95 backdrop-blur-lg flex items-center justify-center p-4 overflow-y-auto" onClick={() => setShowProfileDetail(false)}>
          <div 
            className="w-full max-w-2xl bg-navy-card rounded-[2.5rem] sm:rounded-[3rem] border border-gray-200 p-6 sm:p-8 shadow-card my-auto animate-in zoom-in-95 duration-200"
          ...
          >
            <div className="flex justify-between items-center mb-10">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="text-xl font-black text-gray-900 italic tracking-tighter leading-none">Intelligence Profile</h3>
                  <p className="text-[9px] text-primary font-black uppercase tracking-[0.2em] mt-1">리더십 역량 산출 체계</p>
                </div>
              </div>
              <button onClick={() => setShowProfileDetail(false)} className="size-10 bg-gray-100 rounded-full flex items-center justify-center hover:bg-gray-100">
            ...
            <div className="space-y-8">
              {COMPETENCY_DETAILS.map((comp) => (
                <section key={comp.key} className="relative group border-l border-gray-200 pl-4">
                  ...
                  <div className="space-y-4">
                    <p className="text-[13px] text-gray-600 leading-relaxed font-bold">{comp.definition}</p>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div className="bg-gray-100 p-4 rounded-xl border border-gray-200">
                        <p className="text-[8px] font-black text-gray-400 uppercase tracking-widest mb-1.5">도출 근거</p>
                        <p className="text-[11px] text-gray-500 leading-relaxed font-medium">{comp.basis}</p>
                      </div>
                    </div>
                  </div>
                </section>
      ...
      {showCommunity && (
        <div className="fixed inset-0 z-[100] bg-black/90 backdrop-blur-md flex items-end justify-center px-4" onClick={() => setShowCommunity(false)}>
          <div className="w-full max-w-md bg-navy-card rounded-t-[3rem] p-8 shadow-card border-t border-gray-200 animate-in slide-in-from-bottom-20 duration-500 overflow-y-auto max-h-[90dvh]" onClick={e => e.stopPropagation()}>
            ...
            {isAnalysing ? (
              <div className="flex flex-col items-center justify-center py-20 gap-6">
                ...
                <p className="text-sm font-bold text-gray-500">데이터를 분석 중입니다...</p>
              </div>
            ) : (
              <div className="space-y-8 pb-8">
                <div className="bg-primary/5 p-6 rounded-3xl border border-primary/20">
                  ...
                  <p className="text-sm text-gray-600 leading-relaxed font-medium italic">"{report}"</p>
                </div>
