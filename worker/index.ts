...
const RATE_LIMITS: Record<string, number> = {
  // 주의: 시뮬 1턴 ≈ 채팅 호출 + 신뢰도 채점 호출 2회, 유료 10턴 완주 + 리포트 ≈ 21회 (무료 5턴 ≈ 11회).
  // 한도가 25 미만이면 시뮬 1판도 완주 불가 → 'free: 20' 이 로그인 무료 사용자의
  // 시뮬을 중간에 끊던 버그였음(게스트 200보다 낮은 비정상값). 게스트 이상으로 상향.
  guest: 200,  // 게스트: IP당 일 200회 (3개 시나리오 체험 + 신뢰도/코칭 API 포함)
  free: 200,   // 로그인 무료: 게스트와 동일 (가입자가 게스트보다 불리하면 안 됨) — 시뮬 ~8회/일
  pro: 500,    // 유료: 반복 훈련 충분히 (시뮬 ~20회/일)
  ultra: 1000,
};
